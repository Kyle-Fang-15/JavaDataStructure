...
class SibTreeNode extends TreeNode {
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
    ...
    if(isValidNode()){
      ...
      SibTreeNode theChild=new SibTreeNode(myTree, item);
      theChild.parent=this;
      if (c<=1 || temp==null) {
        c = 1;

        theChild.nextSibling=temp;
        this.firstChild=theChild;
        return;

      }

      while(temp.nextSibling!=null && c>2){

        temp=temp.nextSibling;
        c--;
      }

      theChild.nextSibling=temp.nextSibling;
      temp.nextSibling=theChild;
